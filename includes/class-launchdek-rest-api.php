...
class LAUNCHDEK_REST_API {
	...
	public static function register_routes() {
		...
		// Workflows.
		register_rest_route( self::NAMESPACE, '/workflows', array(
			array(
				'methods'             => 'GET',
				'callback'            => array( __CLASS__, 'get_workflows' ),
				'permission_callback' => array( __CLASS__, 'can_edit_workflows' ),
			),
			array(
				'methods'             => 'POST',
		...
		register_rest_route( self::NAMESPACE, '/workflows/(?P<id>\d+)', array(
			array(
				'methods'             => 'GET',
		...
		register_rest_route( self::NAMESPACE, '/workflows/import', array(
			'methods'             => 'POST',
			'callback'            => array( __CLASS__, 'import_workflow' ),
			'permission_callback' => array( __CLASS__, 'can_edit_workflows' ),
		) );

		register_rest_route( self::NAMESPACE, '/workflows/(?P<id>\d+)/export', array(
			'methods'             => 'GET',
			'callback'            => array( __CLASS__, 'export_workflow' ),
			'permission_callback' => array( __CLASS__, 'can_edit_workflows' ),
		) );

		register_rest_route( self::NAMESPACE, '/workflows/(?P<id>\d+)/validate-step', array(
			'methods'             => 'POST',
	...
	public static function can_edit_workflows() {
		return LAUNCHDEK_Capabilities::current_user_can( LAUNCHDEK_Capabilities::EDIT_WORKFLOWS );
	}

	public static function can_execute() {
		return LAUNCHDEK_Capabilities::current_user_can( LAUNCHDEK_Capabilities::EXECUTE_WORKFLOWS );
	}
	...
	public static function get_dashboard_stats() {
		return rest_ensure_response( array(
			...
			'active_workflows'     => LAUNCHDEK_Workflow_Repository::count_active(),
	...
	public static function get_workflows( $request ) {
		...
		return rest_ensure_response( LAUNCHDEK_Workflow_Repository::all( $args ) );
	}

	public static function get_workflow( $request ) {
		$wf = LAUNCHDEK_Workflow_Repository::find( absint( $request['id'] ) );
		if ( ! $wf ) {
			return new WP_Error( 'not_found', __( 'Workflow not found.', LAUNCHDEK_TEXT_DOMAIN ), array( 'status' => 404 ) );
		}
		return rest_ensure_response( $wf );
	}

	public static function create_workflow( $request ) {
		$id = LAUNCHDEK_Workflow_Repository::create( $request->get_json_params() );
		if ( ! $id ) {
			return new WP_Error( 'create_failed', __( 'Failed to create workflow.', LAUNCHDEK_TEXT_DOMAIN ), array( 'status' => 500 ) );
		}
		return rest_ensure_response( LAUNCHDEK_Workflow_Repository::find( $id ) );
	}

	public static function update_workflow( $request ) {
		$id = absint( $request['id'] );
		if ( ! LAUNCHDEK_Workflow_Repository::find( $id ) ) {
			return new WP_Error( 'not_found', __( 'Workflow not found.', LAUNCHDEK_TEXT_DOMAIN ), array( 'status' => 404 ) );
		}
		LAUNCHDEK_Workflow_Repository::update( $id, $request->get_json_params() );
		return rest_ensure_response( LAUNCHDEK_Workflow_Repository::find( $id ) );
	}

	public static function delete_workflow( $request ) {
		if ( ! LAUNCHDEK_Workflow_Repository::delete( absint( $request['id'] ) ) ) {
			return new WP_Error( 'delete_failed', __( 'Failed to delete workflow.', LAUNCHDEK_TEXT_DOMAIN ), array( 'status' => 500 ) );
		}
		return rest_ensure_response( array( 'deleted' => true ) );
	}

	public static function import_workflow( $request ) {
		$data = $request->get_json_params();
		...
		if ( ! is_array( $data ) ) {
			return new WP_Error( 'invalid_json', __( 'Invalid workflow JSON.', LAUNCHDEK_TEXT_DOMAIN ), array( 'status' => 400 ) );
		}

		$id = LAUNCHDEK_Workflow_Repository::import( $data );
		if ( ! $id ) {
	...
	public static function export_workflow( $request ) {
		$export = LAUNCHDEK_Workflow_Repository::export( absint( $request['id'] ) );
		if ( ! $export ) {
			return new WP_Error( 'not_found', __( 'Workflow not found.', LAUNCHDEK_TEXT_DOMAIN ), array( 'status' => 404 ) );
		}
	...
	public static function start_batch_runs( $request ) {
		...
		if ( ! $workflow_id || empty( $site_ids ) ) {
			return new WP_Error(
				'missing_params',
				__( 'Workflow ID and at least one site ID are required.', LAUNCHDEK_TEXT_DOMAIN ),
				array( 'status' => 400 )
			);
		}

		return rest_ensure_response( LAUNCHDEK_Workflow_Runner::start_batch( $workflow_id, $site_ids ) );
	}
	...
	public static function start_run( $request ) {
		$data = $request->get_json_params();
		$result = LAUNCHDEK_Workflow_Runner::start(
			absint( $data['workflow_id'] ?? 0 ),
	...
	public static function execute_next_step( $request ) {
		$result = LAUNCHDEK_Workflow_Runner::execute_next( absint( $request['id'] ) );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
		return rest_ensure_response( $result );
	...
	public static function execute_auto_steps( $request ) {
		return rest_ensure_response( LAUNCHDEK_Workflow_Runner::execute_all_auto( absint( $request['id'] ) ) );
	}
	...
	public static function complete_manual_step( $request ) {
		...
		LAUNCHDEK_Workflow_Runner::check_completion( absint( $request['id'] ) );
		return rest_ensure_response( array(
			'success' => $ok,
			'run'     => LAUNCHDEK_Run_Repository::find( absint( $request['id'] ) ),
		) );
	...
	public static function get_templates() {
		...
		$db      = LAUNCHDEK_Workflow_Repository::all( array( 'is_template' => true ) );
		return rest_ensure_response( array(
			'builtin' => $builtin,
			'stored'  => $db,
		) );
	...
	public static function save_to_vault( $request ) {
		$data = $request->get_json_params();
		$id   = absint( $data['workflow_id'] ?? 0 );
		if ( ! $id ) {
			return new WP_Error( 'missing_id', __( 'Workflow ID required.', LAUNCHDEK_TEXT_DOMAIN ), array( 'status' => 400 ) );
		}
